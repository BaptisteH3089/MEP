...
plt.legend()
# plt.savefig('/Users/baptistehessel/Documents/DAJ/MEP/DocumentationTeXShop/PresentationFinale/Images/Repartition_ACP_articles.pdf')
...
#plt.savefig('Repartition_ACP_rubriques.pdf')


#%%

##############################################################################
#                    REPRESENTATION PAGES WITH PCA                           #
##############################################################################
...
list_pages = []
for id_page, dict_page in dict_pages.items():
    for i, (ida, dicta) in enumerate(dict_page['articles'].items()):
        vect_art = np.array([dicta[x] for x in list_features], ndmin=2)
        if i == 0:
            vect_page = vect_art
        else:
            vect_page = np.concatenate((vect_page, vect_art))
    list_pages.append(np.sum(vect_page, axis=0))

X_pages = np.array(list_pages)
# We should standardize
scaler = StandardScaler()
X_pages_sc = scaler.fit_transform(X_pages)


# We use PCA to reduce the number of components
X_pages_pca = pca.transform(X_pages_sc)
plt.scatter(X_pages_pca[:, 0], X_pages_pca[:, 1], s=1)

# Difficult to say something about that
# There are some outliers and the rest of the pages is reparted as a triangle
...
list_hist = []
for idp, dicop in dict_pages.items():
    nb_arts = len(dicop['articles'])
    if nb_arts > 7:
        list_hist.append(8)
        dict_nb['7+'] = dict_nb.get('7+', 0) + 1
    else:
        list_hist.append(nb_arts)
        dict_nb[nb_arts] = dict_nb.get(nb_arts, 0) + 1
...
list_hist.sort()
list_bar = [list_hist.count(i) for i in set(list_hist)]

rbar = np.array(list_bar) / len(list_hist)

plt.bar(range(len(rbar)), rbar, width=0.3, edgecolor='black', linewidth=0.5)
plt.xticks(np.arange(0, 8), ['1', '2', '3', '4', '5', '6', '7', '8+'])
plt.xlabel('Pages with n articles')
